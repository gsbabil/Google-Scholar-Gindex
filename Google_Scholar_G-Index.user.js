...
// @exclude        https://scholar.google.com/citations?view_op=list_updates*
...
var config = {
  ...
  'duplicate_citation_selector': 'a.gsc_a_acm',
  'google_total_citation_selector': '#gsc_rsb_st > tbody > tr:nth-child(1) > td:nth-child(2)',
  'location_hostname_selector': 'scholar.google.',
  'citation_indices_table_selector': '#gsc_rsb_st > tbody > tr:nth-child(1)',
...
function main() {
  ...
  if (window.jQuery) {
    ...
    if (location.hostname.indexOf(config.location_hostname_selector) <
      0 &&
    ...
    if (location.href.indexOf("pagesize") < 0 ||
      ...
      // window.clearTimeout(main_timer);
      location.href = location.origin + "/citations?user=" +
        user_id +
    ...
    jQuery(document).ready(function() {
      ...
      load_next_page_timer =
        window.setInterval(loadAllCitationPages,
          1000);
...
function addGindex() {
  ...
  citation_links.each(function(i, link) {
    ...
    if (total_cites >= (g * g)) {
      debugLog("num_citation_in_link=" +
        num_citation_in_link +
        " total_cites=" + total_cites + " g=" +
        g + " g*g=" +
        (g * g));
      gindex = g;
    }
  ...
  duplicate_citation_links.each(function(i, link) {
    ...
    duplicate_cites = duplicate_cites +
      num_duplicate_in_link;
  });
  ...
  html = '<tr>' +
    '<td class="gsb_rsb_sc1">Publications with Citations</td>' +
  ...
  html = '<tr>' +
    '<td class="gsb_rsb_sc1">Total Publications</td>' +
  ...
  html = '<tr>' +
    '<td class="gsb_rsb_sc1">Total Counted Citations</td>' +
  ...
  html = '<tr>' +
    '<td class="gsb_rsb_sc1">Duplicate Citations</td>' +
  ...
  html = '<tr>' +
    '<td class="gsb_rsb_sc1">Effective Total Citations</td>' +
    '<td class="gsc_rsb_std">' + effective_total_cites +
    '</td>' +
    '<td class="gsc_rsb_std">n/a</td>' + '</tr>';
  jQuery(html).appendTo(
    jQuery(config.citation_indices_table_selector).last().parent()
  ...
  if (effective_total_cites != google_total_cites) {
    html =
      ...
      '</td></tr>';
    jQuery(html).appendTo(
      jQuery(config.citation_indices_table_selector).last()
      .parent());
  }
...
function spinnerCss() {
  jQuery('div[id*="spinner_"]')
    .css({
      'position': 'absolute',
      'top': jQuery(window).scrollTop() + 37.5,
      'left': 11.25,
      'padding': '0.1em',
      'z-index': '100000',
      'height': '16px',
...
function loadNextPage() {
  if (jQuery("head").data("loadNextPage_inprogress") != 1) {
    ...
    if (next.length > 0) {
      ...
      var cstart =
        href.replace(new RegExp('.*cstart=(\\d+)$', 'i'),
          '$1');
      ...
      if (href != prev_href) {
        ...
        jQuery.ajax({
          ...
          .done(function(html) {
            var new_content =
              jQuery("form#citationsForm", jQuery(
                html));
            jQuery("span#" + id).first().replaceWith(
              new_content);
            jQuery("head").data(
              "loadNextPage_inprogress", 0);
            spinner.remove();
          });
        // load_next_page_timer =
        //                    window.setTimeout(loadNextPage,1000);
      }
    } else {
      jQuery("head").data("all_pages_loaded", 1);
    }
